...
# --- Descrição que aparece na web ---
import streamlit as st
st.write(f"<span style='font-size: 30px;'>Visualizador de Movimento Uniforme (M.U.)</span>", unsafe_allow_html=True)
st.write('Este é um simulador interativo para visualizar a trajetória de um objeto em Movimento Uniforme. Ajuste a velocidade, a posição inicial e o o tempo na barra lateral esquerda e clique em Ininiar Animaçao.')
...
with col_grafico:
   # st.write("Gráfico de Posição vs. Tempo")
    grafico_placeholder = st.empty()

with col_contadores:
    st.write(f"<span style='font-size: 26px;'>Valores</span>", unsafe_allow_html=True)
    #st.header("Valores")
